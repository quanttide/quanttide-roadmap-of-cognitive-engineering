...
Thought {
    id: Uuid,
    raw: String,            // 用户原始文本
    tags: Vec<String>,      // 模糊标签（clarify 产出）
    created: DateTime,      // 输入时间
}
```
...
1. Tags 应该由谁确定？clarify 阶段由 LLM 生成，还是用户手动标注？
2. Thought 是否需要关联到已存在的 Situation？如果有模糊关联（"这条想法可能属于组织管理"），关联信息存在哪里？
